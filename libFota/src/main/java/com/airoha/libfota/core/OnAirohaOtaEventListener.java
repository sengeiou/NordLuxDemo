...
public interface OnAirohaOtaEventListener {
     ...
     */
    void OnHandleOtaDisabled(final byte errorCode);

    void OnBinFileParseException();

    /**
     *
     * @param throughput Bytes/s
     */
    void OnReportProgramThroughput(float throughput);

    void OnBatteryLevel(int batteryLevel);

    void OnRetryFailed();

    void OnStatusError(final byte errorCode, final String errorMsg);

    void OnOtaServiceNotFound();
}
